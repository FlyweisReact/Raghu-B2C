...
const NewCoursePage = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const AddCart  =async (e) => {
      try{
        const data  = await axios.post("https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/cart" , { courseId :  courseData?.data?._id ,qua: 1, pri:  courseData?.data?.actualprice ,  name:courseData?.data?.level })
        console.log(data)
      }catch(err){
        console.log(err)
      }
