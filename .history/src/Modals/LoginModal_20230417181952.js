...
function LoginModal(props) {
  ...
  const verifyEmail = async (e) => {
    e.preventDefault()
    try { 
      const { data } = await axios.post("https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/forgetpassword" , {email})
      alert(data?.message)
  ...
  const OtpVerify = async  (e) => {
    e.preventDefault()
    try{
      const { data } = await axios.post("https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/verify" , {otp})
    }catch(e) { 
      console.log(e)
      alert(e?.response?.data?.message)
    }
