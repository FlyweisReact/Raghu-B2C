...
import img1 from "./images/Screenshot (2) 1.png";
...
const Cart = () => {
  ...
  const removeHandler= async (id) => {
    try{  
      const data = await axios.put(`https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/cart`, { 
        courseId : id
      })
      alert('item removed')
      fetchHandler()
        ...
        <div className="upper">
          <div className="left">
            ...
            {data?.items?.map((i, index) => (
              ...
              <button onClick={() => removeHandler(i.course)} style={{color : 'red' , border : 'none' , fontSize : '18px ' , cursor : 'pointer'}}>Remove</button>
