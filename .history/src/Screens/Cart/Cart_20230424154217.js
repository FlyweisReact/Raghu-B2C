...
const Cart = () => {
  ...
  const totalPrice = Math.round((perc / 100) * total);
  const actualPrice = totalPrice ? parseInt(totalPrice) : parseInt(total);
  const [courseIdd, setC] = useState("");
  ...
  const DiscountHandler = (e) => {
    couponData?.forEach((obj) => {
      if (e.target.value === obj.code) {
        setPerc(obj.discount);
      }
    });
  };
      ...
      <div className="bigCart">
        ...
        <div className="upper">
          ...
          <div className="right">
            ...
            <div className="sub">
              <p style={{ fontSize: "20px" }}> Total Savings ({perc}%): </p>
