...
const Wishlist = () => {
  ...
  const getData = async () => {
    try{
      const { data } = await axios.get(` https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/wishlist/63e0ea02684e2ce80729ac5f`)
      setData(data.message.course)
      console.log(data)
    }catch(err){
      console.log(err)
    }
