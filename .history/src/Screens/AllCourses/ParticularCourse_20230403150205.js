...
const ParticularCourse = () => {
  ...
  const fetch = useCallback ( async () => {
    try{
      const { data } = await axios.get(`https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/study/${id}`)
      setData(data)
    ...
    <LoginModal show={modalShow} onHide={() => setModalShow(false)} />
      ...
      <div className="ParticularCourseThree">
        ...
        <div className="bottom">
          <p>Full Course</p>
          <div>
            <i className="fa-solid fa-heart" onClick={() => openLoginModal()} style={{cursor : 'pointer'}} > </i>
            <img src={img} alt="" onClick={() => navigate("/cart")} />
          </div>
        </div>
      </div>
