...
const AllCourseNew = () => {
  ...
  const Register = async (e) => {
    e.preventDefault()
    try{
      const { data } = await axios.post("https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/demo/register" , { email , mobile , name , course , slot})
    }catch(err){
      console.log(err)
    }
  }
    ...
    <Header />
      ...
      {data?.message?.map((i , index) => (
        i.levels.map((a , index) => (
          ...
          <img src={a.image} alt={a.level} onClick={() => navigate("/NewCoursePage")} />
        ...
        <form>
          ...
          <div className="two">
            <select>
              ...
              {data?.message?.map((i , index) => (
                i.levels.map((a , index) => (
                  <option value={a.title} key={index} >{a.title} </option>
                ))
              ))}
            </select>
            <input type="date" placeholder="Select Time Slot" />
          </div>
          <button
            type="button"
