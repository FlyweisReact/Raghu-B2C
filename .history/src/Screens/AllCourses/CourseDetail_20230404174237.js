    ...
    <>
      ...
      <div className="CourseDetail">
        ...
        <ul>
          ...
          {section?.result?.map((item, videoNumber) =>
            item?.content?.map((i, index) => (
              <li key={index} className={videoNumber > 4 ? "BlurSection" : ""}>
                {" "}
                {i.section}{" "}
              </li>
            ))
        ...
        {open ? (
          <div className="document">
            {section?.result?.map((item) =>
              item?.content?.map((i, index) =>
                i.resources?.[0]?.name ? (
                  <a href="https://google.com" target='_blank'   rel="noreferrer" key={index}>
                  <p style={{color : 'bl'}}>{i.resources?.[0]?.name}</p>
                  </a>
                ) : (
                  ""
                )
      ...
      <div className="courseDetail3">
        {section?.result?.map((item, videoNumber) =>
          item?.content?.map((i, index) => (
            <div
              className={videoNumber >= 5 ? "main BlurSection2 " : "main"}
              key={index}
              onClick={() => {
                if (videoNumber <= 4) {
                  setVideoUrl(i.video);
