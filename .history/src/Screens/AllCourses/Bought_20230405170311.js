...
const Bought = () => {
  ...
  const fetch2 = useCallback(async () => {
    try {
      const { data } = await axios.get(
        `https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/recomdation/${levelId}`
      );
      setBought(data);
      setCourse(data.result[0]._id);
      setPrice(data.result[0].levels[0].price);
    } catch (e) {
      console.log(e);
    }
  }, [levelId]);
  ...
  const AddAlltoCart = async () => {
    try {
      const { data } = await axios.post(
        "https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/cart/all",
        {
          userId: UserId,
          courseId: course,
          price
  ...
  return (
    <>
      ...
      <div className="boughtone">
        ...
        <div className="right">
          <p className="CourseName" style={{ textTransform: "capitalize" }}>
            {" "}
            {title} level course
          </p>
          <video controls src={data?.result?.[0]?.video}>
